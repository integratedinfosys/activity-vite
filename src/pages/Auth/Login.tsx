...
export default function Login() {
    ...
    const onSubmit: SubmitHandler<Inputs> = async formData => {
        ...
        if (error) {
            modalInfo.current.modalHeader = 'Error occurred in loggin in!'
            modalInfo.current.modalContent = error.message
            modalInfo.current.okClicked = null
            setOpenMessageModal(true);
        } else {
            toast("Logged In! Welcome!")
        }
        setLoading(false);
    };
                        ...
                        <form className="mb-4" onSubmit={handleSubmit(onSubmit)}>
                            ...
                            <div className="mb-6">
                                <div className="flex justify-between mb-2">
                                    ...
                                    <Link to="/forgot-password" className="text-sm text-gray-400 focus:outline-none focus:text-indigo-500 hover:text-indigo-500 dark:hover:text-indigo-300">Forgot password?</Link>
                                </div>
                            ...
                            <p className="text-sm text-center text-gray-400">
                                ...
                                <Link to="/register" className="px-1 font-semibold text-indigo-500 focus:text-indigo-600 focus:outline-none focus:underline">Register</Link>
                            </p>
                        </form>
                        {/*  seperator  */}
